...
def init(directory):
    ...
    path_sum = pd.DataFrame(columns=('path', 'root', 'leave'))
...
def build_enhanced_df(trace_df):
    df_en = build_call_stack(trace_df)
    df_sorted = df_en.sort_values('entry_ts')
    root = df_sorted.iloc[0]['e_name']
    trace_ts = df_sorted.iloc[0]['entry_ts']
    df_graph, df_edge, graph, df_path = build_DAG(df_en, trace_ts)
    df_edge["edge"] = df_edge['source'] + df_edge['target']
    df_edge.to_csv(str(trace_ts), encoding='utf-8')
    print(root)
    return df_edge, df_path, trace_ts
...
def build_DAG(enhanced_df, trace_ts):
    e_list = []
    graph_list = []
    m_list = []
    dict={}
    df_duration = pd.DataFrame(columns=('caller', 'callee', 'edge', 'duration', 'r_dur', 'entry_ts','exit_ts'), dtype=int)
        ...
        if enhanced_df["e_type"].iloc[row] == "ex":
            ...
            if row in enhanced_df.index:
                ...
                if caller_duration != 0:
                    caller_r_dur = 1 / caller_duration
                else:
                    caller_r_dur=0
                ...
                dict = {'caller': caller_string, 'callee': callee_string, 'edge': (caller_string, callee_string),
                        'duration': caller_duration, 'r_dur': caller_r_dur, 'entry_ts': edge_entry_ts, 'exit_ts': edge_exit_ts}
                ...
                graph_list.append((caller_string, callee_string, caller_duration, caller_r_dur, edge_entry_ts, edge_exit_ts))

    graph = nx.from_pandas_edgelist(df_duration, source='caller', target='callee', edge_attr=['duration', 'r_dur', 'entry_ts', 'exit_ts'],
    ...
    plt.savefig(str(trace_ts) + ".png", format="PNG")
    # clearing the current plot
    plt.clf()
    strname= str(trace_ts)+'circular'
    ...
    print(nx.shortest_path(graph, weight='r_dur'))
    df_path= find_longest_path(graph, trace_ts)
    return df_graph, df_edge, graph, df_path


def find_longest_path(graph, trace_ts):
    paths = []
    ...
    roots= list(roots)
    leaves= list(leaves)
    df_path = pd.DataFrame(columns=('path', 'root', 'leave'), dtype=int)

    for r in range(len(roots)):
        for l in range(len(leaves)):
            try: #I should inverse the weight of each edge and calculate shortest bath instead of longest path!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!111
                path= nx.shortest_path(graph, source=roots[r], target=leaves[l], weight='r_dur')
                paths.append(path)
...
def check_hiccup_edge(df2, t):
    ...
    df_sorted.to_csv("df_sorted", encoding='utf-8')
    hiccups = pd.concat(HiccupDict.values(), ignore_index=True)
    hiccups.to_csv("hiccup.txt", encoding='utf-8')
    return hiccups, df_sorted
...
def linear_regression(df):
    df_grouped = pd.DataFrame(columns=('edge', 'duration', 'r_dur', 'source', 'target'))
    df_ramp_edge = pd.DataFrame(columns=('edge', 'slope', 'intercept', 'MSE', 'Root_mean_squared_error', 'R2_score'))
    time_window_size = 10000000000
    ...
    for w in range(0, num_of_windows):
        ...
        df_filtered_grouped = df_filtered.groupby(['edge'], as_index=False).agg(mean_dur = pd.NamedAgg(column="duration", aggfunc="mean"), frequency = pd.NamedAgg(column="edge", aggfunc="count"))
